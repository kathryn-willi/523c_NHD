...
[Figure 2: Schematic of HUC units, taken from the USGS, [<https://nas.er.usgs.gov/hucs.aspx>] ( <https://nas.er.usgs.gov/hucs.aspx>).]
...
Beyond the `tidyverse` will be using `sf` and `mapview` to display geospatial data, and the `nhdplusTools` package to explore the NHD. We will also be using `kableExtra` to make embedded, scrollable data tables.
...
# a HUC-8 using a point or polygon feature 
nofo_watershed <- get_huc8(id = '02070006')
...
nofo_catchments <- get_nhdplus(AOI = nofo_watershed,
 ...
 # about commenting out the below line of code!
...
## Calculatig distances wtih `get_path_length()`
...
```{r}
# Trace the NHDPlus digital elevation model to the nearest downslope  
# NHD flowline
...
Using functoins in `nhdplusTools`, categorize each gage in the the NF River watershed as measuring along the mainstem (i.e., directly on the North Fork of the Shenandoah) or a tributary, without just using the station name to discern which is which. (HINT: What does `get_UM()` do?)

```{r}
mainstem_flowlines <- get_UM(network = nofo_flowlines,
                             comid = 8441255)

gages_categorized <- gages %>% 
  mutate(type = ifelse(comid %in% mainstem_flowlines == TRUE,
                       "Mainstem",
                       "Tributary")) 
```
...
Using the USGS gage "South Platte River Near Kersey, CO" (USGS-06754000)'s watershed as a starting point, create a map that displays only flowline features that represent the Cache la Poudre River's journey as it travels from its start in Rocky Mountain National Park (comid = 2902889), to its confluence with the South Platte River in Greeley Colorado. Also include a shapefile of the Poudre River's watershed on the map. Here's the start of your workflow:

```{r}
platte_ws <- get_nldi_basin(list(featureSource = "nwissite", featureID = "USGS-06754000"))

platte_ws_flowlines <- get_nhdplus(AOI = platte_ws)
```

# Poudre:
```{r}
poudre <- get_DM(platte_flowlines, comid = 2902889) %>%
  get_nhdplus(comid = .) %>%
...
poudre_ws <- get_UT(platte_flowlines, comid = pour_point$comid) %>%
...
Perform a similar trace from the headwaters of the Big Thompson River (comid = 13546) to its confluence with the South Platte River. Also include a sf feature of the Big Thompson River's watershed.
```{r}
big_thompson <- get_DD(platte_flowlines, comid = 13546) %>%
  get_nhdplus(comid = .)

mapview(big_thompson)
```
...
Select one of the following HUC-8's and develop an interactive table of gages within its watershed that includes: distance upstream of the HUC-8 outlet, the stream order the gage is located on, and the gage's contributing watershed area.

Upper Oconnee River, GA (03070101); Concord River, MA (01070005); St. Vrain River, CO (10190005); or Crowley Lake, CA (18090102)

```{r}
# These HUC-8 have enough data to make this exercise worthwhile/comparable for each student.
```
