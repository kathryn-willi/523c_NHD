...
Using functions in `nhdplusTools`, categorize each gage in the the NF Shenandoah River watershed as measuring along the mainstem (i.e., directly on the North Fork of the Shenandoah) or a tributary, without just using the station name to discern which is which. (HINT: What does `get_UM()` do?)
...
```{r}
ds_gage <- filter(gages_categorized, station_nm == "N F SHENANDOAH RIVER NEAR STRASBURG, VA")$comid	
us_gage <- filter(gages_categorized, station_nm == "N F SHENANDOAH RIVER AT COOTES STORE, VA")$comid

dataset <- get_UM(nofo_flowlines, ds_gage) %>%
  as_tibble() %>%
  rename(comid=value)
...
mapview(winnowed_network) + mapview(filter(gages_categorized, type == "Mainstem"))

distance <- get_pathlength(winnowed_network) %>%
  filter(ID == us_gage)

zwischen <- get_DM(nofo_flowlines, us_gage) %>%
  as_tibble() %>%
  rename(comid=value) %>%
  inner_join(dataset, by="comid")

between <- nofo_flowlines %>%
  filter(comid %in% zwischen$comid)

# Mapping just the flowlines between the two:
mapview(between) + mapview(gages_categorized)
```
